...
#include <vanir-xorg-tray-defs.h>
...
#define SOCKET_ADDRESS  "/var/run/xf86-vanir-socket"

#define VANIR_GUI_PROTOCOL_VERSION_LINUX (1 << 16 | 1)
...
void process_xevent_selection(Ghandles * g, XSelectionEvent * ev)
{
    ...
    unsigned char *data;
    Atom Clp = XInternAtom(g->display, "CLIPBOARD", False);
    Atom Qprop = XInternAtom(g->display, "VANIR_SELECTION", False);
...
void mkghandles(Ghandles * g)
{
    ...
    g->xserver_fd = -1;
    wait_for_unix_socket(g);	// wait for Xorg vanir_drv to connect to us
    do {
    ...
    XChangeProperty(g->display, g->stub_win, g->net_wm_name, g->utf8_string_atom,
            8, PropModeReplace, (unsigned char*)"Vanir", 5);
        ...
        char *gui_domid_str)
{
    ...
    switch (pid) {
        ...
        case 0:
            ...
            for (fd = 3; fd < 256; fd++)
                close(fd);
            execl("/usr/bin/vanir-run-xorg", "vanir-run-xorg", NULL);
            perror("execl cmd");
            exit(127);
        default:
            return pid;
...
{
    uint32_t version = VANIR_GUI_PROTOCOL_VERSION_LINUX;
...
void usage()
{
    fprintf(stderr, "Usage: vanir_gui [options]\n");
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        /* first process possible vanir_drv reconnection, otherwise we may be
         * using stale g.xserver_fd */
        if (FD_ISSET(g.xserver_fd, &retset)) {
            char discard[64];
            int ret;

            /* unexpected data from vanir_drv, check for possible EOF */
            ret = read(g.xserver_fd, discard, sizeof(discard));
            if (ret > 0) {
                fprintf(stderr,
                        "Got unexpected %d bytes from vanir_drv, something is wrong\n",
                        ret);
                exit(1);
            } else if (ret == 0) {
                fprintf(stderr,
                        "vanir_drv disconnected, waiting for possible reconnection\n");
                close(g.xserver_fd);
                wait_for_unix_socket(&g);
                wait_fds[1] = g.xserver_fd;
            } else {
                perror("reading from vanir_drv");
                exit(1);
            }
        }
