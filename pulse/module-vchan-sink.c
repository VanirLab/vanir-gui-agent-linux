...
#include "vanir-vchan-sink.h"
#include <libvchan.h>

PA_MODULE_AUTHOR("Cybertrigo");
...
#define DEFAULT_PROFILE_NAME "vanir-vchan-profile"
...
static int do_conn(struct userdata *u)
{
	...
	u->play_ctrl = libvchan_server_init(0, VANIR_PA_SINK_VCHAN_PORT, 128, 2048);
	if (!u->play_ctrl) {
		pa_log("libvchan_server_init play failed\n");
		return -1;
	}
    /* FIXME: 0 is remote domain ID */
	u->rec_ctrl = libvchan_server_init(0, VANIR_PA_SOURCE_VCHAN_PORT, 2048, 128);
	if (!u->rec_ctrl) {
...
static void create_card_ports(struct userdata *u, pa_hashmap *card_ports)
{
	...
	name_prefix = "vanir-vchan-io";
	input_description = "Vanir vchan Input";
	output_description = "Vanir vchan Output";
...
static pa_card_profile *create_card_profile(struct userdata *u, pa_hashmap * ports)
{
	...
	cp = pa_card_profile_new(DEFAULT_PROFILE_NAME, "Default Vanir vchan Profile", 0);
	cp->priority = 20;
	cp->n_sinks = 1;
	cp->n_sources = 1;
...
static int create_card(pa_module *m, struct userdata *u)
{
	...
	pa_proplist_sets(data.proplist, PA_PROP_DEVICE_DESCRIPTION, "Vanir vchan");
	pa_proplist_sets(data.proplist, PA_PROP_DEVICE_STRING, "Vanir vchan");
	pa_proplist_sets(data.proplist, PA_PROP_DEVICE_CLASS, "sound");

	pa_card_new_data_set_name(&data, "Vanir vchan");
...
int pa__init(pa_module * m)
{
	...
	pa_proplist_setf(data_source.proplist, PA_PROP_DEVICE_DESCRIPTION, "Vanir VCHAN source");
